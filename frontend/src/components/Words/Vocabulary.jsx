...
const Vocabulary = ({ words }) => {
  ...
  useEffect(() => {
    const levels = Array.from(new Set(words.map(w => w.nivel))).sort((a, b) => a - b);
  ...
  const getFilteredAndOrderedWords = () => {
    let filtered = words;
  ...
  const updateResults = (wordsList) => {
    setResults(wordsList);
    setCurrentPage(1);
    setShowResults(true);
  };
  ...
  const handleSearch = () => {
    ...
    const filtered = getFilteredAndOrderedWords();
  ...
  const handleShowAll = () => {
    ...
    const all = getFilteredAndOrderedWords();
    updateResults(all);
  };
